...
export const sequenceOf = <T extends any[]>(
...
export const between = <TL, TR>(left: Parser<TL>, right: Parser<TR>) => <T>(
	...
	sequenceOf(tuple(left, content, right)).map(
		(results) => results[1]
	) as Parser<T>
...
const __join = <T extends any[], TP>(
	...
	return sequenceOf(joinedParsers, min).mapError((from) => ({
		...from.error,
		combinator: "join"
	}))
}
...
export const contextual = <T, TResult>(
	generator: () => Generator<Parser<T>, TResult, T>
): Parser<TResult> => {
	return Parser.void.chain(() => {
		...
		const runStep = (next?: T): Parser<TResult> => {
			...
			if (iteration.done) return succeed(iteration.value)
...
export const stateContextual = <T, TResult>(
	generator: () => Generator<Parser<T>, ParserState<TResult>, ParserState<T>>
): Parser<TResult> => {
	return new Parser((inputState) => {
		...
		const transformStep = (next: ParserState<T>): ParserState<TResult> => {
			...
			if (!(nextParser instanceof Parser))
				throw new Error(
					"stateContextual: yielded values must always be parsers"
				)
