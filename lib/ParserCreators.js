...
exports.str = (s) => new Parser_1.Parser((inputState) => {
    if (inputState.targetString.slice(inputState.index).startsWith(s))
        ...
        return inputState.errorify((targetString, index) => `Tried to match "${s}", but got "${targetString.slice(index, s.length + index)}" instead.`);
});
/**
...
exports.letters = exports.reg(/^[A-Za-z]+/)
