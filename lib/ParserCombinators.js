...
//
exports.between = (left, right) => (content) => exports.sequenceOf([left, content, right]).map((results) => results[1]);
/**
...
exports.join = (parsers, joiner, min = -1, joinResults = false) => {
    ...
    for (let parser of parsers) {
        if (starts)
            ...
            parser = exports.sequenceOf([joiner, parser]).map((result) => result[1]);
        }
