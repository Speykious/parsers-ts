...
const randomDigits = (length: number) => {
	if (length <= 0) throw new Error(`The length has to be greater than 0.`)
	...
	return new Parser<[string, string]>((inputState) => {
		...
		if (issl === 0)
			...
			return inputState.errorify(
				`... You got the length just right, but your guess is wrong!`
			)
		else
			return inputState.update(
				issl,
				tuple("Congratulations, you finally got it right!", randits)
			)
	})
}
...
test("Random digits: no guess", () => {
	...
	for (let i = 2; i < 10; i++)
		expect(regnoguess.test(randomDigits(i).run("").error.info)).toBe(true)
})
...
test("Random digits: guess too short", () => {
	...
	for (let i = 2; i < 10; i++)
		expect(
			regtooshort.test(randomDigits(i).run(getRandits(i - 1)).error.info)
		).toBe(true)
})
...
test("Random digits: guess too long", () => {
	...
	for (let i = 2; i < 10; i++)
		expect(
			regtoolong.test(randomDigits(i).run(getRandits(i + 1)).error.info)
		).toBe(true)
})
...
test("Random digits: guess until correct", () => {
	...
	for (let i = 0; i < 100; i++) {
		...
		if (!state.result) expect(wrong.test(state.error.info)).toBe(true)
		else break
	}
