...
test("The Great State-Contextual Parser", () => {
	...
	expect(state1.error).toBe(undefined)
	expect(state2.error).toBe(undefined)
	expect(state3.error).toBe(undefined)
	expect(state4.error).toBe(undefined)
	expect(state5.error).toBe(undefined)
})
